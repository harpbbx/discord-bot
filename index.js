  ...
  if (reason === 'reason_purchase') {
    const components = [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('open_purchase_form').setLabel('🎮 Fill Purchase Form').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('refund_info').setLabel('ℹ️ Refund Info').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('close_ticket').setLabel('🔒 Close Ticket').setStyle(ButtonStyle.Danger)
      )
    ];
    await channel.send({ content: `✅ You have agreed to the rules. You can now use the buttons below.`, components });
  } else {
    // SUPPORT: mostra form supporto, senza fill purchase form
    const embed = new EmbedBuilder()
      .setTitle('❓ What do you need help with?')
      .setDescription('Please describe your issue using the form below.\n\nOur team will get back to you as soon as possible!')
      .setColor(0xE67E22);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('open_support_form').setLabel('📝 Describe your Issue').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('refund_info').setLabel('ℹ️ Refund Info').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('close_ticket').setLabel('🔒 Close Ticket').setStyle(ButtonStyle.Danger)
    );
    await channel.send({ content: `✅ You have agreed to the rules.`, embeds: [embed], components: [row] });
  }
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  // ── BUTTON: refund info ──
  if (interaction.isButton() && interaction.customId === 'refund_info') {
    const embed = new EmbedBuilder()
  ...
  // ── BUTTON: open support form ──
  if (interaction.isButton() && interaction.customId === 'open_support_form') {
  ...
  // ── BUTTON: open purchase form ──
  if (interaction.isButton() && interaction.customId === 'open_purchase_form') {
    const session = userSessions.get(interaction.user.id);
    if (!session?.rulesAccepted) return interaction.reply({ content: '❌ You must accept the rules first.', ephemeral: true });
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'modal_purchase_form') {
    ...
    userSessions.set(interaction.user.id, session);
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('choose_credentials').setLabel('🔐 Use Credentials').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('choose_qr').setLabel('📱 Use QR Code').setStyle(ButtonStyle.Secondary)
    );
    await interaction.reply({ content: `✅ Purchase form submitted!\n\nPlease choose how you want to provide your login information:`, components: [row] });
  }

  // ── BUTTON: choose credentials ──
  if (interaction.isButton() && interaction.customId === 'choose_credentials') {
    const session = userSessions.get(interaction.user.id);
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'modal_credentials') {
    ...
    await interaction.reply({ content: '✅ Credentials saved. The owner will now set the price.', ephemeral: true });
  }
  ...
  if (interaction.isButton() && interaction.customId === 'choose_qr') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x5865F2);
    // MODIFICA 3: testo bottone cambiato in "Confirmed"
    const confirmRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('confirm_qr_login').setLabel('✅ Confirmed').setStyle(ButtonStyle.Success)
    );
  ...
  // ── BUTTON: confirm QR login ──
  if (interaction.isButton() && interaction.customId === 'confirm_qr_login') {
    const session = userSessions.get(interaction.user.id);
    if (!session) return interaction.reply({ content: '❌ Session expired.', ephemeral: true });
    session.loginConfirmed = true;
    userSessions.set(interaction.user.id, session);
    ...
    await interaction.reply({ content: '✅ Login confirmed! The owner will now set the price.', ephemeral: true });
  }
  ...
  // ── MODAL: set price — MODIFICA 6: mostra USD e crypto ──
  if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_set_price_')) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('💰 Price Set')
      .setDescription(
        `The price for the requested games has been set to **${priceEur} EUR** (≈ **$${cryptoAmounts.usdAmount} USD**).\n\n` +
        `**Equivalent in crypto (live rates):**\n` +
        `💵 USDC: **${cryptoAmounts.USDC}** USDC\n` +
        `₿ BTC: **${cryptoAmounts.BTC}** BTC\n` +
        `⟠ ETH: **${cryptoAmounts.ETH}** ETH\n` +
        `◎ SOL: **${cryptoAmounts.SOL}** SOL`
      )
      .setColor(0x57F287)
      .setFooter({ text: 'Rates are live — amounts may vary slightly at time of payment.' });
    ...
    if (customer) {
      ...
      if (userChannel) {
        const choosePaymentRow = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('choose_payment').setLabel('💳 Choose Payment Method').setStyle(ButtonStyle.Success)
        );
        await userChannel.send({
          content: `${customer}`,
          embeds: [new EmbedBuilder()
            .setTitle('💶 Price Confirmed')
            .setDescription(
              `The total for your order is **${priceEur} EUR** (≈ **$${cryptoAmounts.usdAmount} USD**).\n\n` +
          ...
          components: [choosePaymentRow]
        });

        // MODIFICA 7: avvia timer 30 minuti
  ...
  if (interaction.isButton() && interaction.customId === 'choose_payment') {
    const session = userSessions.get(interaction.user.id);
    if (!session?.priceEur) return interaction.reply({ content: '❌ Price not set yet.', ephemeral: true });
    if (!session.loginConfirmed) return interaction.reply({ content: '❌ Login information not provided yet.', ephemeral: true });
    const embed = new EmbedBuilder()
  ...
  // ── PAYPAL FLOW ──
  if (interaction.isButton() && interaction.customId === 'payment_paypal') {
    const session = userSessions.get(interaction.user.id);
    if (!session) return interaction.reply({ content: '❌ Session expired.', ephemeral: true });
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x003087).setFooter({ text: 'PayPal • Instant Payment' });
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('customer_paid_paypal').setLabel('✅ I have paid').setStyle(ButtonStyle.Success));
    await interaction.reply({ embeds: [embed], components: [row], files: [{ attachment: './paypal-qr.png', name: 'paypal-qr.png' }] });
  }

  if (interaction.isButton() && interaction.customId === 'customer_paid_paypal') {
    const session = userSessions.get(interaction.user.id);
    if (!session) return interaction.reply({ content: '❌ Session expired.', ephemeral: true });
